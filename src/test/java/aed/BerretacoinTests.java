...
public class BerretacoinTests {
	...
	public void montoMedio() {
		Transaccion[] transaccionesMontoMedio = new Transaccion[] {
				new Transaccion(0, 0, 1, 1), // 1 -> $2, 2 -> $1
				new Transaccion(1, 1, 2, 2), // 2 -> $3
				new Transaccion(2, 2, 3, 8), // 3 -> $3
				new Transaccion(3, 3, 1, 5), // 1 -> $2, 3 -> $1
				new Transaccion(4, 1, 2, 1), // 1 -> $1, 2 -> $1, 3 -> $1
				new Transaccion(5, 2, 3, 1) // 1 -> $1, 3 -> $2
		};

		berretacoin.agregarBloque(transaccionesMontoMedio);
		assertEquals(5, berretacoin.cantidadTransacciones());
		assertEquals(17, berretacoin.sumaMontos());
		assertEquals(3, berretacoin.montoMedioUltimoBloque());

		berretacoin.hackearTx();
		assertEquals(4, berretacoin.cantidadTransacciones());
		assertEquals(9, berretacoin.sumaMontos());
		// monto medio
